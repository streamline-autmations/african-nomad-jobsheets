...
export interface LineItemInput {
  ...
  row?: number;
  /**
   * Storage paths (in the `job-sheet-files` bucket, under `line-photos/`) of
   * photos attached to this line — reference shots of the actual item, not
   * generic job-sheet attachments. Optional and usually absent; see
   * jobSheetRows.SheetRow.photoPaths for how a row's photos are attributed to
   * whichever side of it actually exists.
   */
  photoPaths?: string[];
}
